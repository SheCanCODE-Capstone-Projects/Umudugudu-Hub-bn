...
const addUser = async (req,res,next)=>{
    try {
        ...
        if (existUser){
            ...
            signUpLink=`<h1> click the link to finish sign up </h1> <a href="http://localhost:4000/api/UH/v1/user/auth/signup">Sign in to continue</a>`;
...
const updateUser = async (req,res,next)=>{
    try {
        var id =req.query.id;
        var toUpdate = await UserModel.findOneAndUpdate({_id:id},req.body);
